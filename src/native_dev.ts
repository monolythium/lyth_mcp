...
export const nativeDevToolDescriptors: NativeDevToolDescriptor[] = [
  {
...
export function registerNativeDevTools(server: McpServer): void {
  for (const descriptor of nativeDevToolDescriptors) {
    server.tool(
      descriptor.name,
      descriptor.description,
      nativeDevToolInputSchema(descriptor.name),
      async (input) => text(await nativeDevToolResult(descriptor, input)),
    );
  }
}
...
export function assertNativeDevProfileOnly(toolNames: readonly string[]): void {
  assertNativeDevTextOnly(toolNames, "tool name");
  assertNativeDevTextOnly(nativeDevProfileStrings(), "profile text");
}

export function nativeDevProfileStrings(): readonly string[] {
  return [
...
function assertNativeDevTextOnly(values: readonly string[], label: string): void {
  const blocked = [
    String("EV") + "M",
    String("er") + "c",
    String("x") + "402",
...
export async function runNativeDevTool(name: string, input: unknown) {
  const descriptor = nativeDevToolDescriptors.find((tool) => tool.name === name);
  if (!descriptor) throw new Error(`unknown native developer tool '${name}'`);
  return nativeDevToolResult(descriptor, input);
}

function nativeDevToolInputSchema(toolName: string): z.ZodRawShape {
...
async function nativeDevToolResult(descriptor: NativeDevToolDescriptor, input: unknown) {
  const command = resolveMonoDevCommand();
  const args = command ? monoDevArgs(descriptor.name, input as Record<string, unknown>) : undefined;
  if (command && args) {
...
interface MonoDevCommand {
  executable: string;
  prefixArgs: string[];
}

function resolveMonoDevCommand(): MonoDevCommand | undefined {
  ...
  if (dir) {
    ...
    for (const candidate of candidates) {
      if (exists(candidate)) return commandForPath(candidate);
    }
  }
  return undefined;
}
...
function commandForPath(path: string): MonoDevCommand {
...
function monoDevArgs(toolName: string, input: Record<string, unknown>): string[] | undefined {
...
function runMonoDev(command: MonoDevCommand, args: string[]) {
