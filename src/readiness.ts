  ...
  return [
    ...
    gate({
      ...
      percent: hasAll(ctx, [
        ...
        "Travala hosted-MCP proxy tools remain read-only while payment stays external.",
        "Coinsbee interim path via NOWPayments-issued invoices.",
      ],
      missing: [
        "Native Monolythium commerce payment path for hosted vendor checkouts.",
        "Direct Coinsbee reseller API (partnership-gated, no fabricated endpoints).",
        "Live verification of Travala hosted MCP response shapes.",
      ],
    }),
    gate({
      id: "tests",
