...
import { checkEvmCap, configureEvmLowValuePolicy, createEvmWallet, deleteEvmWallet, draftEvmDrain, draftEvmFundingRequest, EVM_CHAINS, evmWalletStoreInfo, exportEvmPrivateKey, getEvmWallet, importEvmWallet, isAddress as isEvmAddress, listEvmWallets, moveEvmAccounting, pauseEvmWallet, reserveEvmCap, updateEvmAgentMetadata, } from "./evm_wallet.js";
import { buildErc20Approve, buildErc20Transfer, buildEvmNativeTransfer, evmTokenInfo, probeEvmEndpoints, readErc20Allowance, evmRpcEndpoints, } from "./evm_tx.js";
...
function compactEvmWallet(wallet) {
    return {
        name: wallet.name,
        address: wallet.address,
        algorithm: wallet.algorithm,
...
server.tool("evm_wallet_create", "Create an explicit low-value EVM agent operating wallet (secp256k1). For paying external crypto-commerce vendors only — never custody.", {
    name: z.string().min(1),
    ...
    const wallet = await createEvmWallet({
        ...
        lowValue: lowValueCaps && lowValueCaps.length > 0 ? { enabled: true, caps: lowValueCaps } : undefined,
        revealPrivateKey,
        overwrite,
    });
    const receipt = await addReceipt({
    ...
    return text({
        ...
        receipt,
        warning: "This is an explicitly authorized low-value EVM hot wallet. Fund it only with the operating budget approved for this purpose. ERC-20 + x402 builders land in P14.1 / P14.2.",
    });
});
server.tool("evm_wallet_import", "Import an existing secp256k1 private key into a new low-value EVM agent wallet.", {
    name: z.string().min(1),
...
server.tool("evm_wallet_list", "List local EVM agent wallets (does not expose private keys).", {}, async () => {
    const wallets = await listEvmWallets();
    return text({ wallets: wallets.map(compactEvmWallet) });
});
server.tool("evm_wallet_get", "Get a single EVM agent wallet by name.", { name: z.string().min(1) }, async ({ name }) => {
    const wallets = await listEvmWallets();
...
server.tool("evm_wallet_store_info", "Show EVM wallet store path, count, and file mode for diagnostics.", {}, async () => text(await evmWalletStoreInfo()));
server.tool("evm_wallet_limits", "Update agent metadata and/or low-value caps for an EVM operating wallet.", {
    name: z.string().min(1),
    ...
    if (disableLowValue) {
        ...
        await configureEvmLowValuePolicy({ name, enabled: true, caps: lowValueCaps });
    }
...
server.tool("evm_wallet_pause", "Pause an EVM agent wallet: disable low-value local signing and mark paused.", { name: z.string().min(1), confirm: z.literal("PAUSE_EVM_WALLET") }, async ({ name }) => {
...
server.tool("evm_wallet_delete", "Delete a local EVM agent wallet record after explicit confirmation.", {
    name: z.string().min(1),
    confirmName: z.string().min(1).describe("Must exactly equal name."),
...
server.tool("evm_wallet_fund_request", "Draft a human-readable request to fund an EVM agent wallet for a bounded task.", {
    name: z.string().min(1),
...
server.tool("evm_wallet_drain_draft", "Stub: build a drain draft from an EVM agent wallet to a destination address. Real ERC-20 + native builders land in P14.1.", {
    name: z.string().min(1),
    chainId: z.number().int().positive(),
...
server.tool("evm_wallet_export_private_key", "Export the secp256k1 private key for an EVM agent wallet. Requires explicit confirmation; do not paste into shared contexts.", {
    name: z.string().min(1),
    ...
    return text({
        ...
        address: wallet.address,
        privateKey,
        warning: "Treat this private key as a secret. Anyone with it can spend any funds in this EVM wallet. Do not paste into chat or commit it.",
    });
});
// ---------------------------------------------------------------------------
// EVM transfer + ERC-20 builders (P14.1)
// ---------------------------------------------------------------------------
server.tool("evm_rpc_health", "Probe configured EVM RPC endpoints for a chain id and report latency/sync.", { chainId: z.number().int().positive() }, async ({ chainId }) => {
...
server.tool("evm_token_list", "List canonical EVM token addresses known to the MCP (Circle/Tether issuer-published).", { chainId: z.number().int().positive().optional() }, async ({ chainId }) => text({ tokens: listEvmTokens(chainId) }));
server.tool("evm_native_transfer", "Build (and optionally sign / broadcast) a native ETH transfer from an EVM agent wallet. Submit requires LYTH_MCP_ENABLE_EVM_SUBMIT=1.", {
    walletName: z.string().min(1),
    chainId: z.number().int().positive(),
    ...
    if (!chain)
        return errorText(`unsupported chain ${chainId}`);
...
server.tool("erc20_transfer", "Build (and optionally sign / broadcast) an ERC-20 transfer from an EVM agent wallet. Submit requires LYTH_MCP_ENABLE_EVM_SUBMIT=1.", {
    walletName: z.string().min(1),
    chainId: z.number().int().positive(),
...
server.tool("erc20_approve", "Build an exact-amount ERC-20 approve from an EVM agent wallet. Defaults to exact amount, not unlimited.", {
    walletName: z.string().min(1),
    chainId: z.number().int().positive(),
    asset: z.string().min(1),
    spender: z.string(),
...
server.tool("erc20_allowance", "Read the on-chain ERC-20 allowance owner→spender for an asset on a chain.", {
    chainId: z.number().int().positive(),
    asset: z.string().min(1),
    owner: z.string(),
    spender: z.string(),
    ...
    if (!isEvmAddress(owner))
        return errorText(`invalid owner: ${owner}`);
    if (!isEvmAddress(spender))
        return errorText(`invalid spender: ${spender}`);
...
// ---------------------------------------------------------------------------
server.tool("x402_vendor_policy_set", "Create or update an x402 vendor policy: origin allowlist, allowed assets, per-request caps, and the EVM wallet that pays.", {
    ...
    if (!wallet)
        return errorText(`evm wallet '${walletName}' not found`);
    const policy = {
        vendorId,
        walletName,
        originAllowlist,
...
server.tool("x402_vendor_policy_list", "List configured x402 vendor policies.", {}, async () => text({ policies: await listX402Policies() }));
server.tool("x402_vendor_policy_get", "Get one x402 vendor policy by id.", { vendorId: z.string().min(1) }, async ({ vendorId }) => text({ policy: await getX402Policy(vendorId) }));
server.tool("x402_vendor_policy_remove", "Remove an x402 vendor policy after explicit confirmation.", {
    vendorId: z.string().min(1),
    confirm: z.literal("REMOVE_X402_POLICY"),
}, async ({ vendorId }) => text(await removeX402Policy(vendorId)));
server.tool("x402_pay", "Fetch a URL; if the server returns HTTP 402, sign an EIP-3009 USDC authorization from the configured vendor wallet and retry with the X-PAYMENT header. Requires a matching x402 vendor policy.", {
...
server.tool("agent_identity_set_local", "Set the local ERC-8004 agent identity used by vendor connectors that support attribution (e.g. Travala's agentId / rewardWallet).", {
...
server.tool("agent_identity_get", "Get the locally configured ERC-8004 agent identity (agentId + rewardWallet) used by vendor connectors.", {}, async () => {
...
// ---------------------------------------------------------------------------
server.tool("travala_info", "Show how lyth_mcp talks to Travala: hosted MCP URL, agent identity wiring, x402 payment path.", {}, async () => {
    const identity = await readAgentIdentity();
    return text({
        travalaMcpUrl: travalaMcpUrl(),
...
server.tool("travala_proxy_call", "Forward an arbitrary tool call to Travala's hosted MCP (e.g. travala_search_hotel, travala_search_package, travala_manage_bookings, travala_cancel_booking). Read-only by design; booking + payment uses travala_book_pay.", {
    tool: z.string().min(1),
...
server.tool("travala_book_recover", "Look up a Travala booking that errored or timed out, BEFORE retrying travala_book_pay. Avoids double-charge.", {
    packageId: z.string().min(1),
    sessionId: z.string().min(1),
...
server.tool("agent_identity_register_guide", "Explain how to obtain an ERC-8004 agentId for use with vendor connectors that support attribution (e.g. Travala cbBTC giveback).", {}, async () => text({
    steps: [
    ...
    note: "No on-chain register_draft tool is provided yet. The ERC-8004 reference contracts are deployed on Ethereum Sepolia in the upstream repo (Jan 2026 spec); a verified Base-mainnet IdentityRegistry address has not been published for direct on-chain tx drafting. Once an authoritative Base-mainnet IdentityRegistry address is published, this MCP can ship an agent_identity_register_draft tool that builds the registration tx directly.",
}));
